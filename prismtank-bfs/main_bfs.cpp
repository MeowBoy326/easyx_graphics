...
void map_search(int targx, int targy)
{
	if (targx >= 0 && targy >= 0 && targx < WIDTH && targy < HEIGHT && !map[targx][targy].searched)
	{
		...
		if (targx == endx && targy == endy)
		{
			...
			sprintf(text, "成功,共%d步", map[endx][endy].depth);
